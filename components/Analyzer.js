...
export default class Analyzer{
    ...
    adjustFrequency(i, avr){
      var f = Math.max(0, this.spectrums[this.sourceStart + i] - avr) * this.scale*0.4;
    ...
    update(){
      ...
      this.avr = (!this.audio.isReady || this.avr === 0) ? this.avr : Math.min(40, Math.max(this.avr, -50));
    ...
    calcFrequency(num, interval, i){
      ...
      } else if(this.controls.props.isTrilinear){
        targetFrequency = this.trilinear( p0, p1, p2);
    ...
    trilinear(p1,p2,p3){
      ...
      return(1 - p1*0.5)* (1 - p2*0.5)* (1 - p3*0.6)  
